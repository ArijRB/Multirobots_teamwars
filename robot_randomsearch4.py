...
#    print "Evaluation #", evaluationIt
    ...
    for i in range(genomeSize):
        params_.append(sigma*gauss(0,1)+params[i])
    
    for i in range (nbReevaluations): # N évaluations indépendantes
        fit_, nb_= agents[0].evaluate(params_)
        fitness += fit_
        nbCases += nb_
    ...
    if bestFitness < fitness:
        ...
        sigma = sigma * pow(2,-1/4)
#    print "\tParameters:", str(params)
    genomes.append(params)
...
while True:
    ...
    # evalue les parametres
    selec = input('choisissez un genome a jouer : ')
    fitness = agents[0].evaluate(genomes[selec])
