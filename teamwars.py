...
from teamwars_agentA import *  # EQUIPE A
...
def stepWorld():
    ...
    for i in range(len(agents)):
        agents[shuffledIndexes[i]].step(sensors)
        # met à jour la grille d'occupation
        coord = agents[shuffledIndexes[i]].getRobot().get_centroid()
        occupancyGrid[int(coord[0])/16][int(coord[1])/16] = agents[shuffledIndexes[i]].getTeamname() # first come, first served
    return
...
def displayOccupancyGrid():
    ...
    for y in range(screen_height/16):
        for x in range(screen_width/16):
            if occupancyGrid[x][y] == AgentTypeA.teamname:
                nbA = nbA+1
                sys.stdout.write('A')
            elif occupancyGrid[x][y] == AgentTypeB.teamname:
                nbB = nbB+1
                sys.stdout.write('B')
            else:
                nothing = nothing + 1
    ...
    sys.stdout.write('\t'+AgentTypeA.teamname+': ')
    sys.stdout.write(str(nbA))
    sys.stdout.write('\n')
    sys.stdout.write('\t'+AgentTypeB.teamname+': ')
